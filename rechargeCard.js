...
function deleteCard(cardId) {
    ...
    let cardToDelete = database.find(card => card.id === cardId);
    // let confirmDelete = confirm(`You're about to trash ${database[i].network}, Are you sure?`)
    let confirmDelete = confirm(`You're about to trash ${cardToDelete.network}, Are you sure?`);
    if (confirmDelete) {
        // database.splice(i, 1)
        database = database.filter(card => card.id !== cardId);
        updateStorage();
        searchCards();
        console.log(database);
        console.log(cardId);
...
function fillTable() {
    tbody.innerHTML = '';
    database.forEach(({ id, network, logo, amount, pin, used, date }, index) => {
        ...
        tbody.innerHTML += ` 
            <tr style="${rowStyle}">
                ...
                <td>${date}</td>
                <td style = "background-color: rgb(233, 26, 26); cursor: pointer; text-shadow: 1px 0 5px #FFFFFF;" title="Delete Card" onclick="deleteCard(${id})"> 🗑️ </td>
            </tr> `
    })
}
...
function displayFiltered(dataToShow) {
    ...
    dataToShow.forEach(({ id, network, logo, amount, pin, used, date }, index) => {
        let rowStyle = used ? "background-color: rgba(255, 0, 0, 0.2);" : "";
        ...
        <tr style="${rowStyle}">
            ...
            <td>${used ? "Used ✅" : "Unused ❌"}</td>
            <td>${date}</td>
            <td style="background-color: rgb(233, 26, 26); cursor: pointer;" title="Delete Card" onclick="deleteCard(${id})"> 🗑️ </td>
        </tr> `;
    });
}
console.log(database)